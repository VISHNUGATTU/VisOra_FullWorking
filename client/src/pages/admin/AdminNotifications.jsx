...
const AdminNotifications = () => {
  ...
  useEffect(() => {
    const delayDebounce = setTimeout(async () => {
      if (searchQuery.length > 1 && !formData.isBroadcast) {
        try {
          const endpoint = formData.targetRole === 'Faculty' ? '/api/faculty/search' : '/api/student/search';
          const res = await axios.get(`${endpoint}?q=${searchQuery}`);
          if (res.data.success) {
            setSearchResults(formData.targetRole === 'Faculty' ? res.data.faculty : res.data.students);
          }
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) {
        toast.success("Notifications Sent Successfully!", { id: loadingToast });
  ...
  const handleMarkAsRead = async (id) => {
    ...
    try {
      ...
      const res = await axios.put(`/api/notifications/read/${id}`);
      if (!res.data.success) {
  ...
  const handleMarkAllRead = async () => {
    ...
    try {
      await axios.put('/api/notifications/read-all', { role: 'Admin' });
        ...
        <div className="sm:flex sm:items-center sm:justify-between mb-8">
          <div>
            <h1 className="text-2xl font-semibold text-gray-900 flex items-center gap-2">
              <Bell className="text-gray-500" size={24} /> Notifications
            </h1>
            <p className="mt-1 text-sm text-gray-500">Manage system alerts and broadcast communications.</p>
          </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-0">
            ...
            <div className="relative bg-white rounded-xl shadow-xl max-w-lg w-full transform transition-all p-6 sm:p-8">
              ...
              <div className="flex justify-between items-center mb-5">
                <h3 className="text-lg font-semibold text-gray-900">Compose Alert</h3>
              ...
              <form onSubmit={handleCreateSubmit} className="space-y-5">
                    ...
                    <select 
                      ...
                      onChange={(e) => {
                        ...
                        setFormData({
                          ...
                          targetRole: newRole, 
                          selectedUsers: [],
                          isBroadcast: newRole === 'Student' ? true : formData.isBroadcast 
                        });
                      }}
                    >
                ...
                <div>
                  ...
                  <div className="flex items-center gap-6 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <label className={`flex items-center gap-2 text-sm ${formData.targetRole === 'Student' ? 'text-gray-400 cursor-not-allowed' : 'text-gray-900 cursor-pointer'}`}>
                      <input 
                        type="radio" 
                        className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 disabled:opacity-50"
                        disabled={formData.targetRole === 'Student'}
                        checked={!formData.isBroadcast} 
                     ...
                     <div className="space-y-3 mt-3">
                        ...
                        <div className="relative">
                          ...
                          <input 
                            ...
                            placeholder={`Search ${formData.targetRole.toLowerCase()} name...`} 
                            value={searchQuery} 
                ...
                <div>
                  ...
                  <textarea 
                    ...
                    placeholder="Provide notification details..." 
        ...
        {showDeleteModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative bg-white rounded-xl shadow-xl max-w-sm w-full p-6 text-center">
              ...
              <p className="text-sm text-gray-500 mb-6">Are you sure you want to delete this notification? This action cannot be undone.</p>
